...
if str(ROOT) not in sys.path:
    sys.path.append(str(ROOT))  # 将根路径添加到路径中
...
class DetectAPI:
    ...
    def detect(self, source):
        ...
        # DataLoader 加载数据
        # 直接从 source 加载数据
        dataset = source  # 直接将视频源传递给 dataset
        ...
        for img in dataset:
            with dt[0]:
                ...
                # print("det 的类型:", type(det))
                # print("det 的长度:", len(det))
                im0 = img.copy()  # copy 一个原图片的副本图片
                ...
                if len(det):
                    ...
                    # 映射预测信息到原图
                    # print(f"Original image shape: {im0.shape}")
                    # print(f"Predictions before scaling: {det[:, :4]}")
                    det[:, :4] = scale_boxes(im.shape[2:], det[:, :4], im0.shape).round()
                    # print(f"Boxes after scaling: {det[:, :4]}")
                    ...
                    try:
                        ...
                        print("An exception occurred while processing detections:", e)
