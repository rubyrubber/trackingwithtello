...
def func1(dx, dy, boundary_x, boundary_y, height, width, numChannels):
    ...
    # Use np.int32 or np.int64 instead of int for Numba compatibility
    alfa = np.zeros((height, width, 2), np.int32) # Modified

    for j in range(1, height-1):  # xrange changed to range for Python 3
        for i in range(1, width-1):  # xrange changed to range for Python 3
            c = 0
            x = dx[j, i, c]
            y = dy[j, i, c]
            r[j, i] = np.sqrt(x*x + y*y)

            for ch in range(1, numChannels):  # xrange changed to range for Python 3
                tx = dx[j, i, ch]
                ty = dy[j, i, ch]
            ...
            for kk in range(0, NUM_SECTOR):  # xrange changed to range for Python 3
...
def getFeatureMaps(image, k, mapp):
	...
	mapp['sizeY'] = sizeY
	mapp['numFeatures'] = p
	# 在 fhog.py 的 getFeatureMaps 函数中，修改数组创建语句
