...
def rearrange(img):
	print("At start of rearrange, img shape:", img.shape)

	assert img.ndim == 2, "Input must be a 2D array."
	img_ = np.zeros_like(img)

	yh, xh = img.shape[0] // 2, img.shape[1] // 2
	yh2, xh2 = img.shape[0] - yh, img.shape[1] - xh

	# 由于形状可能不匹配，使用min函数来保证形状一致性
	y_min = min(yh, yh2)
	x_min = min(xh, xh2)

	print("Before quadrant swap, calculated mid points and adjustments:", yh, xh, yh2, xh2, y_min, x_min)

	# 执行象限交换
	img_[0:y_min, 0:x_min] = img[yh:yh + y_min, xh:xh + x_min]
	img_[yh:yh + y_min, xh:xh + x_min] = img[0:y_min, 0:x_min]

	img_[0:y_min, xh:xh + x_min] = img[yh:yh + y_min, 0:x_min]
	img_[yh:yh + y_min, 0:x_min] = img[0:y_min, xh:xh + x_min]

	print("After quadrant swap, img_ shape:", img_.shape)
...
class KCFTracker:
	...
	def gaussianCorrelation(self, x1, x2):
		if(self._hogfeatures):
			...
			for i in range(self.size_patch[2]):
				...
				#caux = rearrange(caux)
				c += caux
			print("Before rearrange call, c shape:", c.shape)
			c = rearrange(c)
			print("After rearrange call, c shape:", c.shape)
		else:
	...
	def getFeatures(self, image, inithann, scale_adjust=1.0):
		...
		if(z.shape[1]!=self._tmpl_sz[0] or z.shape[0]!=self._tmpl_sz[1]):
			...
			self._tmpl_sz = [int(self._tmpl_sz[0]), int(self._tmpl_sz[1])]
			print(f"self._tmpl_sz: {self._tmpl_sz}, type: {type(self._tmpl_sz[0])}, {type(self._tmpl_sz[1])}")
