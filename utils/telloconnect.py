...
class TelloConnect:
    ...
    def __init__(
        ...
        self.sock_cmd = self.socket.socket(
            self.socket.AF_INET,
        ...
        # tello state
        self.sock_state = self.socket.socket(
            self.socket.AF_INET,
            self.socket.SOCK_DGRAM
            )
    ...
    def set_image_size(self, image_size=(920, 720)):
        """Set size of the aptured image

        Args:
            image_size (tuple, optional):
            Retun image size. Defaults to (960,720).
        """
    ...
    def start_communication(self):
        ...
        if self.receiverThread.is_alive() is not True:
            self.receiverThread.start()
        if self.eventThread.is_alive() is not True:
            self.eventThread.start()
        if self.stateThread.is_alive() is not True:
    ...
    def send_cmd_return(self, cmd):
        ...
        Returns:
            [str]: UPD aswer to the emmited command,
            see Tello SDK for valid answers
        """
        # send cmd over UDP
